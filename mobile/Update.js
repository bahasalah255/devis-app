...
export default function Update({ navigation, route }) {
	...
	const [dateEm, setDateEm]       = useState(parseDate(initial.date_emission));
	const [dateVal, setDateVal]     = useState(parseDate(initial.date_validite));
	...
	const totalHT  = lignes.reduce((sum, l) => sum + calcLigne(l), 0);
	...
	const submit = async () => {
		...
		try {
			const token = await AsyncStorage.getItem('token');
			await axios.put(`${API_BASE_URL}/devis/${initial.id}`, {
			...
			Alert.alert('Devis mis a jour', 'Modifications enregistrees.', [
				{ text: 'OK', onPress: () => navigation.replace('Dash') },
			]);
		} catch (e) {
			if (e?.response?.status === 422) Alert.alert('Validation', 'Verifiez les champs.');
			else Alert.alert('Erreur', 'Impossible de mettre a jour.');
	...
	return (
		<SafeAreaView style={s.safe}>
			...
			<View style={s.header}>
				...
				<Text style={s.headerTitle}>{initial.numero}</Text>
				<View style={{ width: 55 }} />
			</View>
			...
			<ScrollView style={{ flex: 1 }} contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
				...
				<View style={s.group}>
					...
					<View style={s.rowSep} />
					<TouchableOpacity style={s.row} onPress={() => setShowStatut(true)}>
						<Text style={s.rowLabel}>Statut</Text>
						<Text style={[s.rowValue, { color: C.accent }]}>{statut}</Text>
					</TouchableOpacity>
				</View>
				...
				<View style={s.sectionRow}>
					...
					<TouchableOpacity onPress={() =>
						setLignes(p => [...p, { produit_id:'', nom:'', description:'', quantite:'1', prix_unitaire:'', remise:'0' }])
					}>
				...
				{lignes.map((ligne, i) => (
					<View key={i} style={[s.group, { marginBottom: 10 }]}>
						...
						<View style={s.row}>
							...
							<TextInput style={s.rowInput} placeholder="Optionnel" placeholderTextColor={C.sub}
								value={ligne.description} onChangeText={v => setLigne(i, 'description', v)} />
						</View>
						<View style={s.rowSep} />
						<View style={s.row}>
							<Text style={s.rowLabel}>Quantite</Text>
							<TextInput style={s.rowInput} keyboardType="numeric" placeholder="1" placeholderTextColor={C.sub}
								value={ligne.quantite} onChangeText={v => setLigne(i, 'quantite', v)} />
						</View>
						<View style={s.rowSep} />
						<View style={s.row}>
							<Text style={s.rowLabel}>Prix unitaire</Text>
							<TextInput style={s.rowInput} keyboardType="numeric" placeholder="0.00" placeholderTextColor={C.sub}
								value={ligne.prix_unitaire} onChangeText={v => setLigne(i, 'prix_unitaire', v)} />
						</View>
						<View style={s.rowSep} />
						<View style={s.row}>
							<Text style={s.rowLabel}>Remise (%)</Text>
							<TextInput style={s.rowInput} keyboardType="numeric" placeholder="0" placeholderTextColor={C.sub}
								value={ligne.remise} onChangeText={v => setLigne(i, 'remise', v)} />
						</View>
						<View style={s.rowSep} />
						...
						{lignes.length > 1 && (
							<>
								...
								<TouchableOpacity style={s.row} onPress={() => setLignes(p => p.filter((_,idx) => idx !== i))}>
									<Text style={{ color: C.danger, fontSize: 15 }}>Supprimer cette ligne</Text>
								</TouchableOpacity>
							</>
						)}
					</View>
				...
				<View style={s.group}>
					...
					<View style={s.rowSep} />
					<View style={s.row}>
						<Text style={[s.rowLabel, { fontWeight: '700' }]}>Total TTC</Text>
				...
				<TouchableOpacity style={[s.btn, submitting && { opacity: 0.6 }]} onPress={submit} disabled={submitting}>
					{submitting ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>Enregistrer les modifications</Text>}
				</TouchableOpacity>
			...
			<DateModal
				...
				onConfirm={d => { dateTarget === 'em' ? setDateEm(d) : setDateVal(d); setDateTarget(null); }}
			...
			<PickerSheet
				...
				data={clients}
				renderItem={item => (
					<TouchableOpacity style={s.sheetItem} onPress={() => { setClientId(String(item.id)); setShowClients(false); }}>
			...
			<PickerSheet
				...
				data={STATUTS.map(st => ({ id: st, nom: st }))}
				renderItem={item => (
					<TouchableOpacity style={s.sheetItem} onPress={() => { setStatut(item.id); setShowStatut(false); }}>
						<Text style={[s.sheetItemMain, statut === item.id && { color: C.accent, fontWeight: '700' }]}>
							{item.nom}
						</Text>
					</TouchableOpacity>
				)}
...
const s = StyleSheet.create({
	...
	group: { backgroundColor: C.white, borderRadius: 12, borderWidth: 1, borderColor: C.border, overflow: 'hidden' },
	row: {
	...
	btn:    { backgroundColor: C.accent, borderRadius: 12, paddingVertical: 15, alignItems: 'center', marginTop: 16 },
	...
	dateSheetBar: { flexDirection: 'row', justifyContent: 'space-between', paddingHorizontal: 20, paddingVertical: 14, borderBottomWidth: 1, borderBottomColor: C.border },
	dateCancel:   { fontSize: 16, color: C.sub },
	dateConfirm:  { fontSize: 16, color: C.accent, fontWeight: '600' },

	sheetOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'flex-end' },
	sheet:        { backgroundColor: C.white, borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 16, maxHeight: '65%' },
	sheetBar:     { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
